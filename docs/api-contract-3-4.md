...
> 当前 UI 规范和接口文档暂列五类任务：`FRONTEND`、`BACKEND`、`DATABASE`、
> `TEST`、`SECURITY`。后端 AI 实现尚未完成正式枚举约束，最终任务类型需与3号
> 后端负责人确认。在正式契约收敛前，前端保持对上述五类任务的兼容，不能删除
> `SECURITY`。最终枚举以正式 Swagger、DTO 和后端实际实现为准；Prompt 不作为
> 接口契约依据。
