...
本轮覆盖跨阶段质量门禁、7/30 天失访预警、JWT 认证、患者/随访/预警归属、管理端权限边界、敏感字段加解密与脱敏导出、临床风险分层及 AI 建议确认/驳回、字典和指标趋势接口、五个新增业务页面、Flyway 历史升级、三角色 API E2E 以及浏览器登录安全闭环。
...
| 失访预警 | 7 天黄色、30 天红色、同周期幂等，随访后自动关闭 | 专项单测、Flyway V7、服务实现 |

## 4. 自动化结果

| 测试层 | 命令/方式 | 结果 |
|---|---|---|
| 后端全量测试 | `mvn --batch-mode verify` | 98 项，0 失败、0 错误、0 跳过 |
| 后端规范门禁 | `mvn --batch-mode checkstyle:check` | 0 违规，严格失败开关已启用 |
| 前端单元测试 | `npm test` | 13 个文件、33 项通过 |
...
| 专项 | 覆盖结果 |
|---|---|
| 临床决策 | 风险分层、AI 建议、确认写入随访、驳回、不存在建议，共 5 项 |
| 敏感字段加解密 | 中文往返、随机 IV、历史明文、空值、缺失/错误密钥，共 4 项 |
| 脱敏导出 | 管理员原文、医生脱敏与责任范围、导出审计元数据，共 3 项 |
| 字典接口 | 正常、空结果、缺少类型，共 3 项 |
| 指标趋势接口 | 指标数据、空结果、缺少患者参数，共 3 项 |
| 前端新增页面 | AI 建议、消息中心、随访计划、随访任务、随访模板，共 10 项 |

### 命令截图证据

- [mvn test](./test-evidence/2026-08-27-maven-test.png)
- [npm test](./test-evidence/2026-08-27-npm-test.png)
- [npm run lint](./test-evidence/2026-08-27-npm-lint.png)
- [npm run build](./test-evidence/2026-08-27-npm-build.png)

## 6. 数据库迁移验收

- 隔离环境：MySQL 9.5，临时端口 33307，不使用本机既有实例。
- 空库启动：Flyway V1–V7 均成功，7 条历史记录均为 `success=1`。
- 历史升级：先执行 V1，写入旧格式失访预警，再执行失访迁移；合并最新 main 后迁移顺延为 V7；旧记录保留，新增 `source_due_date` 为 NULL。
...
交叉自检采用三条独立证据链：服务层单元/边界测试、真实 MySQL 的 API E2E、Chromium 浏览器安全流程。历史失访迁移与候选 V1→V7 全链路分别从隔离库验证，避免只依赖应用启动日志。
