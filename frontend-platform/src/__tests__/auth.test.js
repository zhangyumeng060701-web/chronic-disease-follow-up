...
describe('auth session', () => {
  it('uses the same user identity fields as the target frontend', () => {
    ...
    expect(getCurrentUserName(createStorage({ username: 'doctor', realName: '张医生' }))).toBe('张医生')
  })

  it('clears every shared auth field on logout', () => {
    const storage = createStorage({ token: 't', role: 'ADMIN', username: 'u', realName: 'n', theme: 'light' })
