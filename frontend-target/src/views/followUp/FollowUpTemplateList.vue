<template>
  <div class="template-list">
    ...
    <el-table :data="tableData" v-loading="loading" empty-text="暂无随访模板">
      ...
      <el-table-column prop="defaultContent" label="默认内容" min-width="180" show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="160" fixed="right">
        <template #default="{ row }">
          ...
          <el-button size="small" :type="row.isActive === 1 ? 'warning' : 'success'" @click="handleToggle(row)">
...
const emptyForm = () => ({
  templateCode: '', templateName: '', riskLevel: 'MEDIUM',
  frequencyDays: 14, followUpType: '电话', defaultContent: '', isActive: 1
})
...
function handleReset() { searchForm.templateCode = ''; search(queryParams()) }
...
function handleEdit(row) {
  ...
  Object.assign(formData, {
    templateCode: row.templateCode, templateName: row.templateName, riskLevel: row.riskLevel,
    frequencyDays: row.frequencyDays, followUpType: row.followUpType,
    defaultContent: row.defaultContent || '', isActive: row.isActive
  })
  dialogVisible.value = true
}
